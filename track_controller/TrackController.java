...
import track_controller.communication.Crossing;

import track_controller.communication.CtcLink;
import track_controller.communication.Occupancy;
...

{
	...
	public static int MAX_BRANCH_SIZE = 10;
        ...
        public static int MAX_BINARY_BLOCK_ADDRESS_LENGTH = 10;
	...
	private Authority[] authorities = {
        ...
        private Occupancy[] occupancies = {
            new Occupancy(new boolean[MAX_BRANCH_SIZE]),
        ...
        private Crossing[] crossings = {
            new Crossing(new boolean[MAX_BRANCH_SIZE]),
            new Crossing(new boolean[MAX_BRANCH_SIZE]),
            new Crossing(new boolean[MAX_BRANCH_SIZE])
        };
...

	{
        ...
        //Sets the Occupancies at the specified branch
        
        public void occupancies(Branch branch, boolean[][] blockNumbers)
                
                throws IllegalArgumentException
        {
            int[] blockNumbersInt = Program.parseRunBlocks(blockNumbers);
            boolean[] occs = new boolean[blockNumbersInt.length];
            int index = branchToIndex(branch);
            for(int i = 0; i < blockNumbersInt.length; i++)
            {
                if(blockNumbersInt[i] == 0)
                {
                    occs[i] = true;
                }
                else
                {
                    occs[i] = track_model.getBlock(line, blockNumbersInt[i]).isIsOccupied();
                }
            }
            occupancies[index] = new Occupancy(occs);
            
        }
        
        
        //Sets the Railroad Crossing presence at the specified branch
        ...
        {
            int[] blockNumbersInt = Program.parseRunBlocks(blockNumbers);
            boolean[] crosses = new boolean[blockNumbersInt.length];
            int index = branchToIndex(branch);
            for(int i = 0; i < blockNumbersInt.length; i++)
            {
                if(blockNumbersInt[i] == 0)
                ...
                {
                    crosses[i] = track_model.getBlock(line, blockNumbersInt[i]).isIsCrossing();
                }
            }
            crossings[index] = new Crossing(crosses);
        }
